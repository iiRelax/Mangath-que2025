<html>
   ...
   <?php
      ...
      if (isset($user) && ($user->getRole())->name == "user"):?>
            <?php echo '<h2 class="card-title text-center">Collection</h2>';?>
             <?php 
            foreach($collection as $collec):?> 
            
            <?php 
            $counter=0;
            foreach($collec->getMangas() as $key =>$manga):?> 
            ...
            ?></a>
                 ...
                 <?php if($counter==3 || sizeof($collec->getMangas())-1==$key){echo '</div>';$counter=-1;} ?>
                 <?php $counter++; ?>
          <?php endforeach;?> 
    <?php endforeach;?>
    <?php else:?>
